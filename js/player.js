...
class Player {
    ...
    reset() {
        ...
        this.major = null;           // 专业
        this.year = 1;               // 年级 (1-4)
        this.semester = 1;           // 学期 (1:上学期, 2:下学期)
        this.monthIndex = 0;         // 月份索引 (0-9: 9月-6月)
        this.day = 3;                // 日期
        this.totalActions = 0;       // 总行动次数
        this.actionsLeft = 10;       // 本月剩余行动点

        // 月份配置 (9月-6月，完整学年)
        this.months = ['9月', '10月', '11月', '12月', '1月', '2月', '3月', '4月', '5月', '6月'];
        this.daysInMonth = [30, 31, 30, 31, 31, 28, 31, 30, 31, 30];  // 每月天数
        this.isWinterBreak = false;  // 是否在寒假
        this.justEnteredNewMonth = false;  // 是否刚进入新月份
        ...
        // 技能熟练度系统 (0-100百分制，用于钻研技能)
        this.skillProficiency = {};  // 各技能的熟练度

        // 专业水平 (0-100 百分制) - 决定每学期刷新后的绩点
        this.academic = 30;          // 学业水平起始30

        // 绩点系统 (百分制，起始60分)
        this.gpa = 60;               // 绩点 (0-100)
        ...
        this.monthlyStats = {
            ...
            startAnxiety: this.anxiety,
            startIdealism: this.idealism,
            startGpa: this.gpa,
            startNetwork: this.network,
            actions: [],                // 本月行动记录
            events: [],                 // 本月触发的事件
            skillsLearned: []           // 本月学习的技能
        };
    }
     ...
     */
    getDateString() {
        const yearName = this.getYearName();
        const semesterName = this.semester === 1 ? '上学期' : '下学期';
        return `大${yearName}${semesterName} ${this.months[this.monthIndex]}${this.day}日`;
    }
    ...
    advanceTime() {
        ...
        // 检查生活费是否归零超过一天（即金钱为0且不是刚刚进入新月份）
        if (this.money <= 0 && !this.justEnteredNewMonth) {
            result.outOfMoney = true;
            return result;
        }

        // 推进日期（每次行动推进约3天）
        this.day += 3;
        const maxDay = this.daysInMonth[this.monthIndex] || 30;
        if (this.day > maxDay) {
            this.day = maxDay;
        }

        // 检查是否是12月30日期末考试（每学期第4个月月底）
        if (this.monthIndex === 3 && this.day >= 30 && this.actionsLeft <= 5) {
            result.exam = 'final';
        }
        ...
        if (this.actionsLeft <= 0) {
            ...
            this.monthIndex++;
            this.day = 3;
            this.justEnteredNewMonth = true;

            // 检查是否进入寒假 (1月)
            if (this.monthIndex === 4) {
                this.isWinterBreak = true;
                result.winterBreak = true;
                this.semester = 1; // 保持上学期标识
            }

            // 检查是否春季学期开学 (3月)
            if (this.monthIndex === 6) {
                this.isWinterBreak = false;
                result.springStart = true;
                this.semester = 2; // 下学期
                // 春季学期刷新绩点
                this.refreshGPA();
            }
            ...
            // 每月发放生活费（除了9月入学时已经有生活费）
            if (!(this.year === 1 && this.monthIndex === 0 && this.totalActions <= 10)) {
            ...
            this.justEnteredNewMonth = false;
        }
     ...
     * 检查生活费是否足够活到月底
     */
    checkMoneyForSurvival() {
        // 计算剩余天数（每次行动约3天）
        const actionsRemaining = this.actionsLeft;
        const moneyNeeded = actionsRemaining * 120;
        return {
            canSurvive: this.money >= moneyNeeded,
            moneyNeeded: moneyNeeded,
            shortfall: Math.max(0, moneyNeeded - this.money),
            daysUntilEnd: actionsRemaining * 3
        };
    }

    /**
     * 根据专业水平刷新绩点（百分制）
     * 专业<50: 60分, 50-75: 80分, >75: 90分
     */
    refreshGPA() {
        let baseGPA;
        if (this.academic < 50) {
    ...
    forgetSkill(skillId) {
        if (this.skills[skillId]) {
            ...
            delete this.skillProficiency[skillId];  // 同时删除熟练度
    ...
    practiceSkill(skillId) {
        if (!this.skills[skillId]) {
            return { success: false, message: '未学会该技能' };
        }

        // 初始化熟练度（如果没有）
        if (this.skillProficiency[skillId] === undefined) {
            this.skillProficiency[skillId] = 0;
        }

        // 每次钻研+2熟练度
        const oldProficiency = this.skillProficiency[skillId];
        this.skillProficiency[skillId] = Math.min(100, oldProficiency + 2);
        const newProficiency = this.skillProficiency[skillId];

        // 钻研技能消耗：焦虑+3，理想+1（投入时间钻研带来的压力和小成就感）
        this.anxiety = Math.min(100, this.anxiety + 3);
        this.idealism = Math.min(100, this.idealism + 1);

        return {
            success: true,
            skillId: skillId,
            oldProficiency: oldProficiency,
            newProficiency: newProficiency,
            increase: newProficiency - oldProficiency
        };
    }
     ...
     * 记录月度行动
     */
    recordMonthlyAction(actionType, description) {
        ...
        this.monthlyStats.actions.push({ type: actionType, desc: description });
    }

    /**
     * 记录月度事件
     */
    recordMonthlyEvent(eventTitle) {
        if (this.monthlyStats) {
            this.monthlyStats.events.push(eventTitle);
        }
    }
     ...
     * 记录月度技能学习
     */
    recordMonthlySkill(skillId) {
        if (this.monthlyStats) {
            this.monthlyStats.skillsLearned.push(skillId);
        }
    }

    /**
     * 生成月度总结
     * @returns {Object} 月度总结数据
     */
    generateMonthlySummary() {
        if (!this.monthlyStats) return null;

        const stats = this.monthlyStats;
        const currentMonth = this.months[this.monthIndex] || '本月';

        // 计算变化
        const changes = {
            money: this.money - stats.startMoney,
            anxiety: this.anxiety - stats.startAnxiety,
            idealism: this.idealism - stats.startIdealism,
            gpa: this.gpa - stats.startGpa,
            network: this.network - stats.startNetwork
        };
        ...
        // 生成幽默描述
        let description = this.generateMonthlyDescription(actionCounts, changes);

        // 检查警报
        const alerts = [];
        if (this.network < 10) alerts.push({ type: 'danger', msg: '⚠️ 人脉极低！你已处于社交孤岛状态' });
        if (this.anxiety > 80) alerts.push({ type: 'danger', msg: '⚠️ 焦虑爆表！建议立即躺平' });
        if (this.anxiety > 60) alerts.push({ type: 'warning', msg: '💡 焦虑较高，注意休息' });
        if (this.gpa < 60) alerts.push({ type: 'danger', msg: '⚠️ 绩点不及格！小心退学警告' });
        if (this.money < 500) alerts.push({ type: 'warning', msg: '💡 生活费告急，考虑兼职' });
        if (this.idealism < 30) alerts.push({ type: 'info', msg: '💡 理想主义低落，需要寻找人生意义' });
        ...
        return {
            ...
            eventsTriggered: this.monthlyStats.events,
            skillsLearned: this.monthlyStats.skillsLearned
        };
    }
     ...
     * 生成月度描述
     */
    generateMonthlyDescription(actionCounts, changes) {
        const descriptions = [];
        ...
        if (maxAction) {
            const [type, count] = maxAction;
            const ratio = count / Math.max(totalActions, 1);

            if (type === 'study') {
                if (ratio > 0.5) descriptions.push('这个月你几乎泡在了图书馆，学霸模式全开。');
                else if (ratio > 0.3) descriptions.push('学习占据了你的大部分时间，知识就是力量。');
                else descriptions.push('偶尔去图书馆装装样子，至少态度端正。');
            } else if (type === 'relax') {
                if (ratio > 0.5) descriptions.push('这个月你彻底躺平了，床是你最忠实的伙伴。');
                else if (ratio > 0.3) descriptions.push('休息得不错，就是有点心虚。');
                else descriptions.push('偶尔放松一下，劳逸结合。');
            } else if (type === 'work') {
                if (ratio > 0.5) descriptions.push('打工人的一个月，钱包鼓鼓但身心俱疲。');
                else if (ratio > 0.3) descriptions.push('兼职赚了不少，学业却有点顾不上了。');
                else descriptions.push('赚点外快，改善生活。');
            } else if (type === 'social') {
                if (ratio > 0.5) descriptions.push('社交达人模式，你的微信步数一定很高。');
                else if (ratio > 0.3) descriptions.push('认识了新朋友，人脉圈在扩大。');
                else descriptions.push('偶尔社交，保持联系。');
            } else if (type === 'practice') {
                if (ratio > 0.3) descriptions.push('专注技能提升，为未来投资。');
                else descriptions.push('抽空钻研技能，日积月累。');
            }
        }

        // 根据数值变化添加描述
        if (changes.money > 500) descriptions.push('财运不错，小金库充实了不少。');
        else if (changes.money < -500) descriptions.push('花钱如流水，钱包在哭泣。');

        if (changes.anxiety > 20) descriptions.push('压力山大，焦虑值蹭蹭上涨。');
        else if (changes.anxiety < -10) descriptions.push('心情舒畅，焦虑得到了缓解。');

        if (changes.idealism > 10) descriptions.push('理想主义回升，对未来充满希望。');
        else if (changes.idealism < -15) descriptions.push('现实打击，理想主义有所下滑。');

        if (changes.gpa > 5) descriptions.push('绩点提升，学业有进步！');
        else if (changes.gpa < -5) descriptions.push('绩点下滑，需要加把劲了。');

        // 特殊事件描述
        if (this.monthlyStats.events && this.monthlyStats.events.length > 0) {
            descriptions.push(`本月发生了${this.monthlyStats.events.length}件特别的事。`);
        }

        if (this.monthlyStats.skillsLearned && this.monthlyStats.skillsLearned.length > 0) {
            descriptions.push(`新学会了${this.monthlyStats.skillsLearned.length}项技能！`);
        }

        // 默认描述
        if (descriptions.length === 0) {
            descriptions.push('平平淡淡的一个月，没有什么特别的。');
        }
    ...
    applyChoiceEffects(choiceId) {
        ...
        // 特殊处理学生会工作 - 需要返回标记让game.js显示专门弹窗
        if (choiceId === 'studentUnion') {
            appliedEffects['studentUnionWork'] = true;
            // 应用效果
        ...
        if (choiceId === 'social') {
            ...
            if (Math.random() < 0.3) {
                ...
                // 70%概率：随机选择两种结果之一
                // 消耗金钱：200或300随机
                const moneyCost = Math.random() < 0.5 ? 200 : 300;
                // 增加人脉：+2、+3或+4随机
                const networkGain = [2, 3, 4][Math.floor(Math.random() * 3)];
                // 增加理想：+2、+3或+4随机
                const idealismGain = [2, 3, 4][Math.floor(Math.random() * 3)];

                if (Math.random() < 0.5) {
                    // 结果1：休闲放松
                    appliedEffects['socialResult'] = 1;
                    appliedEffects['socialDesc'] = '休闲放松';
                    this.modifyStat('gpa', -1);
                    this.modifyStat('anxiety', -20);
                    this.modifyStat('money', -moneyCost);
                    this.modifyStat('relations.roommate', 5);
                    this.modifyStat('idealism', idealismGain);
                    this.modifyStat('network', networkGain);
                } else {
                    // 结果2：社交拓展
                    appliedEffects['socialResult'] = 2;
                    appliedEffects['socialDesc'] = '社交拓展';
                    this.modifyStat('gpa', -1);
                ...
                // 记录实际数值用于显示
                appliedEffects['moneyCost'] = moneyCost;
                appliedEffects['networkGain'] = networkGain;
                appliedEffects['idealismGain'] = idealismGain;
            }
            return appliedEffects;
        }
...
你开始了打工生涯，在奶茶店、便利店、外卖站之间流转。偶尔你会梦见自己还在学校上课，醒来后枕头总是湿的。`,
        ...
        // 正常结束（到大四6月）
        if (this.year === 4 && this.monthIndex === 9 && this.actionsLeft === 0) {
...
"也许以后会好起来的。"你对自己说，虽然你自己也不太相信。`,
    ...
    load() {
        ...
        try {
            ...
            if (!this.skillProficiency) this.skillProficiency = {};
